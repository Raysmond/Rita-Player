...
public class JsoupTest {
	 ...
	 * @return
	 */
	public static ArrayList<LRCSearchResult> getSearchResultFromBaidu(
		...
		try {
			...
			Elements songList1 = s.getElementsByTag("ul").first()
					.getElementsByTag("li");
			...
			for (Element e : songList1) {
				...
				Element download = e.getElementsMatchingOwnText("下载LRC歌词")
						.first();
				String linkData = download.className();
				String linkDownload = linkData
						.substring(linkData.indexOf("data2"),
				...
				item.setLrcText(downloadLRC("http://music.baidu.com/"
						+ linkDownload));
